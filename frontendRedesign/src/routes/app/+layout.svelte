<script lang="ts">
	...
	*/
	...
	//creating data a la dure pour tester le front
	$img_path = '/logo.jpeg'
	$clientName = 'dmercadi';
	$userId = 42;
	...
	*/
	...
	/**
	 * Search Block
	 * note : we can use async function to return data to insert in id42NameInputNotEmpty, 
	 * in this case, id42NameInputNotEmpty would be a Promise and we can await in svelte html block to show loading data component
	 */
	let nameSearchInput = "";
	...
	async function getSpecifiedClients()
	{
		...
		searchRes = [{ name: 'abouhlel' }, { name: 'silas' }];
