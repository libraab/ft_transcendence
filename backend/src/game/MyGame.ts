...
function precolision(state: any, player: any) {
    ...
    let m: number = state.ball.velocityY / state.ball.velocityX;
    let b: number = state.ball.y - m * state.ball.x;
    let newx: number = state.ball.x + state.ball.velocityX;
    let newy: number = state.ball.y + state.ball.velocityY;
    if (state.ball.x < 600 / 2) {
        let dist: number = Math.sqrt(Math.pow(newx - (player.x + player.width), 2) + Math.pow(newy - player.y, 2));
        if (dist <= state.ball.radius)
            return 1;
        dist = Math.sqrt(Math.pow(newx - (player.x + player.width), 2) + Math.pow(newy - (player.y + player.height), 2)); 
        if (dist <= state.ball.radius)
            return 1;
        let xcolision: number = /*xmur ?*/0 + player.width + state.ball.radius; 
        if (newx < xcolision && xcolision < state.ball.x) {
            let ycolision: number = m * xcolision + b;
        ...
        let dist: number = Math.sqrt(Math.pow(newx - player.x, 2) + Math.pow(newy - player.y, 2));
        if (dist <= state.ball.radius)
            return 1;
        dist = Math.sqrt(Math.pow(newx - player.x, 2) + Math.pow(state.ball.y - (newy + player.height), 2)); 
        if (dist <= state.ball.radius)
            return 1;
        let xcolision: number = /*xmur ?*/600 - player.width - state.ball.radius; 
